...
if(exercicio < 2022){
   numRecurso = ( arrRecurso.size() >= 4 ? (arrRecurso[0].padRight(1,'0') + "." + arrRecurso[1].padRight(1,'0') + "." + getRight(arrRecurso[2], 2).padRight(2,'0') + "." + arrRecurso[3].padRight(6,'0')) : "" )
} else {
   numRecurso = ( arrRecurso.size() >= 3 ? (arrRecurso[0].padRight(1,'0') + "." + arrRecurso[1].padRight(3,'0') + "." + getRight(arrRecurso[2], 7).padRight(7,'0'))  : "" )
}

def DOT_CodigoResumido = """${numOrgao}.${numUnidade}.${numFuncao}.${numSubfuncao}.${numPrograma}.${numAcao}.${numNatureza}.${numRecurso}"""

def modeloIdeal2021    = "99.999.99.999.9999.99999.9.9.99.99.99.9.9.99.999999"
def modeloIdeal2022    = "99.999.99.999.9999.99999.9.9.99.99.99.9.999.9999999"

println modeloIdeal2021
println modeloIdeal2022
